...
from grepl.scrape.video_clip_downloader import download_clip_from_timestamped_url
...
def get_all_footage_urls() -> list[str]:
    ...
    cursor.execute("""
    SELECT 
        DISTINCT parsed_video_url.timestamped_url
    FROM parsed_page
    INNER JOIN parsed_video_url
    ON parsed_page.video_url = parsed_video_url.video_url
    AND parsed_page.caption = parsed_video_url.caption
    WHERE 
        1=1
        AND timestamped_url IS NOT NULL 
        AND timestamped_url != '' 
        AND parsed_page.tags is not null
        AND parsed_page.tags LIKE '%#footage%' 
    """)
...
def _download(url: str):
    ...
    try:
        download_clip_from_timestamped_url(url)
        return True
